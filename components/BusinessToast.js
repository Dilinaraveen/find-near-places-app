...
function BusinessToast({userLocation}) {
  ...
  return (
    ...
    <div className='fixed bottom-5 right-5 z-30 flex items-center bg-purple-400 p-5 rounded-2xl gap-5'>
      ...
      <div className='bg-purple-300 p-4 rounded-xl hover:scale-105 transition-all cursor-pointer' onClick={()=>onDirectionClick()}>
        <Image src='/send.png' alt='nav' width={30} height={30}/>
      </div>
    </div> : null }
    </>
  )
